...
class ExperienceBuffer(Dataset):
    def __init__(self, num_envs, horizon_length, batch_size, minibatch_size, obs_dim, act_dim, priv_dim, device):
        ...
        self.storage_dict = {
            ...
            'rewards': torch.zeros((self.num_envs, self.transitions_per_env, 1), dtype=torch.float32, device=self.device),
            'values': torch.zeros((self.num_envs, self.transitions_per_env,  1), dtype=torch.float32, device=self.device),
            'neglogpacs': torch.zeros((self.num_envs, self.transitions_per_env), dtype=torch.float32, device=self.device),
            'dones': torch.zeros((self.num_envs, self.transitions_per_env), dtype=torch.uint8, device=self.device),
            'actions': torch.zeros((self.num_envs, self.transitions_per_env, self.act_dim), dtype=torch.float32, device=self.device),
            'mus': torch.zeros((self.num_envs, self.transitions_per_env, self.act_dim), dtype=torch.float32, device=self.device),
            'sigmas': torch.zeros((self.num_envs, self.transitions_per_env, self.act_dim), dtype=torch.float32, device=self.device),
            'returns': torch.zeros((self.num_envs, self.transitions_per_env,  1), dtype=torch.float32, device=self.device),
        }
    ...
    def update_data(self, name, index, val):
        # `index` is the horizon timestep; layout is [N, H, ...] so write the
        # time slice across all envs.
        if type(val) is dict:
    ...
    def computer_return(self, last_values, gamma, tau):
        # Layout: storage_dict[name][:, t, ...]  (N envs along dim 0, time t along dim 1)
        last_gae_lam = 0
